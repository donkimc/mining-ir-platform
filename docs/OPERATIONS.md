...
2. Note a marker: current time (UTC), latest migration name from `src/migrations/index.ts`, and that Qelvarion public `/` loads.
...
6. Smoke without secrets in logs:
   - Public `/` shows Qelvarion Published content only
   - Company Admin login works
   - One Published media file via `/api/media/file/…`
   - Zenthoriq Resource poison strings absent from public HTML/JSON
